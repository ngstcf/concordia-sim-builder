...
Vendored into the simbuilder backend to keep the pinned Concordia fork
(ngstcf/concordia@v2.4.0-simbuilder) unmodified.  Upstream Concordia removed
ParallelQuestionnaireEngine and EntityAgent.stateless_act in commit 030d2fa
("Remove ParallelQuestionnaireEngine and deprecated stateless_act"), migrating
open-ended questionnaires to the asynchronous engine.  The interviewer /
Likert (multiple-choice) questionnaire path used for ICC(3,1) reliability still
needs this engine, which drives the game_master.questionnaire.Questionnaire
component's JSON-list protocol (NEXT_ACTION_SPEC returns one action spec per
player per question) and records answers as scalar per-dimension values.

This is the engine's source as of commit 030d2fa^, with the single call to the
removed EntityAgent.stateless_act replaced by the module-level _stateless_act
helper below (identical behavior, no dependency on the vendored fork).
"""
...
def _stateless_act(
  ...
  """Return an action attempt without advancing phase or writing memory.

  Replicates EntityAgent.stateless_act (removed from the pinned Concordia fork
  in commit 030d2fa).  The caller must have set the agent's phase to PRE_ACT;
  this gathers each component's pre_act context and returns the act component's
  action attempt, deliberately skipping post_act/update so a single agent can
  answer many questionnaire items independently and in parallel.
  """
  if agent.get_phase() != entity_component.Phase.PRE_ACT:
    raise RuntimeError('Agent must be in PRE_ACT phase for stateless act')
  executor = futures.ThreadPoolExecutor()
  try:
    contexts = agent._parallel_call_('pre_act', action_spec, executor=executor)  # pylint: disable=protected-access
  finally:
    executor.shutdown(wait=True)
  return agent._act_component.get_action_attempt(contexts, action_spec)  # pylint: disable=protected-access
      ...
      try:
        for player_name, q_id, spec_str in player_qid_spec_list:
          ...
          def process_question_task(
            ...
            answer = _stateless_act(agent, action_spec)
            with mutex:
