...
"""Context-aware scripted entity prefab.

Unlike the basic_scripted prefab which forces exact responses regardless of
context, this prefab uses script lines as adaptive prompts that guide the
LLM while allowing it to respond to conversation context.

This creates more natural dialogue where the scripted agent can:
...
class Entity(prefab_lib.Prefab):
  """A prefab implementing a context-aware scripted entity."""

  description: str = (
      'An entity that follows a script but adapts responses based on '
      'conversation context. Unlike basic_scripted, this agent can react '
      'to other participants while maintaining the scripted flow.')
  ...
  def build(
    ...
    # Import the custom context-aware scripted act component
    from backend.prefabs.context_aware_scripted_act import ContextAwareScriptedActComponent

    act_component = ContextAwareScriptedActComponent(
        model=model,
        component_order=component_order,
        script=self.params.get('script', []),
        end_statement=self.params.get('end_statement'),
    )
