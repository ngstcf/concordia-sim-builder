...
import { getRecentSimulations, getSimulationLog, getCheckpointFiles, deleteCheckpointFiles, deleteSimulationLog } from '../../utils/api';
...
export default function RecentSimulations({ onLoadSimulation, onResumeSimulation }: RecentSimulationsProps) {
  ...
  const handleDelete = async (e: React.MouseEvent, filename: string) => {
    e.stopPropagation();
    if (!confirm(`Delete "${extractTitle(filename)}"? This cannot be undone.`)) return;
    try {
      await deleteSimulationLog(filename);
